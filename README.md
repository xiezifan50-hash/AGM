...
- `src/codex_ma/config.py`：读取 `multiagent.toml`，解析项目级角色 agent 配置、并发、重试和安全配置。
...
- `codex-ma doctor`：检查 Codex CLI、项目级 agent 配置、schema 和已有任务状态。
...
- Review：输出 review 队列、每个 review verdict、评分、项目路径、评测维度、逐项证据、findings 摘要、聚合结果和建议。feature review 固定为 generator 代码的技术审查；dimension review 只使用 generator 与 evaluator 已协商接受的 `accepted_contract.review_dimensions`。
...
默认 agent 档位按效果与额度消耗做分层：`generator` 使用 `gpt-5.3-codex` + `medium` reasoning 负责真实代码修改；`evaluator` 使用 `gpt-5.4` + `medium` reasoning 负责合同与整体验收；`reviewer` 使用 `gpt-5.4-mini` + `medium` reasoning 支撑并行审查；`orchestrator` 和 `normalizer` 使用 `gpt-5.4-mini` + `low` reasoning 处理轻量编排与 schema 规范化。默认 `generator`、`evaluator`、`reviewer` 开启 `search`，`orchestrator` 和 `normalizer` 保持关闭。
...
测试默认使用 `FixtureRunner`，不依赖真实 Codex CLI 调用。真实运行任务时需要本机可用的 Codex CLI，并在 `multiagent.toml` 中配置好项目级 agent 档位。
...
- 将角色配置从全局 Codex profile 改为项目级 `[agents.<role>]`，并按 generator/evaluator/reviewer/orchestrator 职责设置默认模型与 reasoning 档位以平衡效果和额度消耗。
