...
- [ ] 增加 `sprint0` 和 `planner`：在 `sprint0` 中由 `planner` 生成大方向上的总览计划，至少明确总共预计需要几个 sprint，以及每个 sprint 大致要做什么；让 `planner` 和 `evaluator` 在该阶段反复 argue，以便在 `sprint0` 产出一份可靠的总览计划。同时允许后续 sprint 根据实际进展实时修改该计划，确保计划始终符合实际情况。
- [ ] 增加“半自动模式”：每轮 sprint 结束后插入人工介入环节，允许 agent 索要需要的图片素材、和用户讨论当前方向是否正确，以及确认某些细节是否需要更改；原先不中断运行的流程作为“全自动模式”保留。
